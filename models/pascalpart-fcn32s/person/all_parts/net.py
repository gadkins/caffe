...
def fcn(obj_cls, part, split):
    ...
    n.score_fr = L.Convolution(n.drop7, num_output=25, kernel_size=1, pad=0,
        param=[dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)])
    n.upscore = L.Deconvolution(n.score_fr,
        convolution_param=dict(num_output=25, kernel_size=64, stride=32,
            bias_term=False),
...
def make_net():
	...
	f.write(str(fcn('person', 'all_parts', 'train')))

    with open('val.prototxt', 'w') as f:
        #f.write(str(fcn('val')))
	f.write(str(fcn('person', 'all_parts', 'val')))
